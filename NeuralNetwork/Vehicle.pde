...
class Vehicle {
  ...
  void steer(ArrayList<PVector> targets) {
    ...
    //emcarvaj
    for (int i = 0; i < forces.length; i++) {
      PVector currenttarget= targets.get(i);
      if (abs(location.x - currenttarget.x)<20) 
      {
        velocity.x =velocity.x*-0.1;
      }
      if (abs(location.y - currenttarget.y)<20) 
      {
  ...
  PVector seek(PVector target) {
    ...
    //println( desired.x, desired.y, location.x, location.y);
    //delay(100);
